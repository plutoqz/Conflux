...
> 文档状态：已批准；M0-M2、P0 与 P1 已完成验收；P1.5 已实现并完成离线合同验收，跨领域真实 API 盲评待预算确认；V2 answer_first 管道已替代 P1.5 为默认（config.yaml pipeline: answer_first），P1.5 仍可通过 `pipeline: p15` 使用；P2、M3+ 尚未启动
>
> 版本：1.8
>
> 日期：2026-07-20
...
| P1.5 | 泛化深度研究与内容扩展 | 问题原型、动态领域地图、覆盖矩阵、分层综合、来源路由、泛化报告契约 | 已实现并完成离线合同验收；真实跨领域盲评待预算确认；V2 answer_first 已替代为默认管道 |
| V2 | 简化研究管道 (`answer_first`) | 6步线性 decompose→retrieve→generate→synthesize→audit→finalize | ✅ 已上线为默认；硬编码清理/参数调优/真实盲评待完成 |
